...
public class Vehicle {
  ...
  public boolean isInIntersection() {
    ...
    double safetyMargin = Config.INTERSECTION_HALF + Config.VEHICLE_LENGTH;

    return x >= Config.CENTER_X - safetyMargin
        && x <= Config.CENTER_X + safetyMargin
        && y >= Config.CENTER_Y - safetyMargin
        && y <= Config.CENTER_Y + safetyMargin;
  }
  ...
  public boolean isOffScreen() {
    ...
    return x < -margin
        || x > Config.WINDOW_W + margin
        || y < -margin
        || y > Config.WINDOW_H + margin;
  }
  ...
  /**
   * Lane convention (right-hand driving): moving SOUTH -> x = cx - HALF_LANE moving NORTH -> x = cx
   * + HALF_LANE moving WEST -> y = cy - HALF_LANE moving EAST -> y = cy + HALF_LANE
   *
   * <p>Each turn is a quarter circle: it starts at the stop-line edge, bends around the lane corner
   * (used as the Bezier control point), and ends on the exit-lane edge. This makes right turns
   * tight (they curve BEFORE the center) and left turns sweep wide.
   */
      ...
      case NORTH:
        {
          ...
          if (turn == Turn.STRAIGHT) {
            ...
            addQuarterTurn(coordinates, laneX, cy - edge, laneX, westLane, cx - edge, westLane);
            coordinates.add(new double[] {OFF_LEFT, westLane});
          } else { // LEFT -> moving east
            coordinates.add(new double[] {laneX, cy - edge});
            addQuarterTurn(coordinates, laneX, cy - edge, laneX, eastLane, cx + edge, eastLane);
      ...
      case EAST:
        {
          ...
          if (turn == Turn.STRAIGHT) {
            ...
            addQuarterTurn(coordinates, cx + edge, laneY, northLane, laneY, northLane, cy - edge);
            coordinates.add(new double[] {northLane, OFF_TOP});
          } else { // LEFT -> moving south
            coordinates.add(new double[] {cx + edge, laneY});
            addQuarterTurn(coordinates, cx + edge, laneY, southLane, laneY, southLane, cy + edge);
  ...
  private void addQuarterTurn(
    ...
    for (int step = 1; step <= curveSteps; step++) {
      ...
      double px = inverse * inverse * startX + 2.0 * inverse * t * controlX + t * t * endX;
      double py = inverse * inverse * startY + 2.0 * inverse * t * controlY + t * t * endY;
  ...
  private void updatePosition() {
    ...
    double segmentProgress =
        segmentLength == 0.0 ? 1.0 : (progress - start.distance) / segmentLength;
